#!/usr/bin/env python3
"""Publish the complete MONET corpus using the verified 4M-trained CLIP heads.
...
def verify_inputs():
    ...
    for dim, suffix in ((2, ""), (3, "-3d")):
        folder = SANDBOX / f"monet-clip-fullcorpus-proj-4m{suffix}-20260905"
        manifest = json.loads((folder / "manifest.json").read_text())
        expected = SANDBOX / f"monet-random-clip-4m{suffix}/champion-bs16k/model.pt"
        checkpoint_hash = digest(expected)
        if manifest["checkpoint"] != str(expected) or manifest["checkpoint_sha256_16"] != checkpoint_hash[:16] or manifest["dim"] != dim:
            raise ValueError("Projection must use the matching 4M-trained head")
        coords = np.load(folder / "coords.f32.npy", mmap_mode="r")
        n, pool = manifest["n_rows"], manifest["n_pool"]
        if coords.shape != (n, dim) or manifest["row_layout"] != {"pool": [0, pool], "complement": [pool, n]}:
        ...
        projections.append(dict(path=str(folder / "coords.f32.npy"), sha256=digest(folder / "coords.f32.npy"),
    ...
    if any(projections[0]["manifest"][key] != projections[1]["manifest"][key]
           for key in ("n_rows", "n_pool", "n_complement", "row_layout", "row_alignment")):
...
def main():
    ...
    dataset = f"monet-clip-basemap-full-4m-{args.release}"
    ...
    projections, paths, thumbs_receipt = verify_inputs()
    n = projections[0]["manifest"]["n_rows"]
    ...
    if args.resume and provenance_path.exists():
        ...
        provenance = dict(dataset=dataset, n_points=n, embedding="CLIP ViT-B/32",
            projection="latent-basemap, random 4M-trained 2D and 3D heads", coordinates=projections,
    ...
    if args.resume and (source / "manifest.json").exists():
        ...
        try:
            result = assign_and_build(dataset_id=dataset, points_df=points, coords3d=np.load(projections[1]["path"], mmap_mode="r"),
                ...
                thumb_url_template=MONET_THUMB_URL_TEMPLATE, umap_run="basemap-random-4m-head", points_table_path=folder / "points.parquet",
