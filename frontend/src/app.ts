...
async function bootstrapStreamedWorld(): Promise<void> {
  try {
    ...
    miningController = new MiningController(
      ...
      () => !runtimeProfile.mobile && hotbar.equippedTool === "pickaxe",
      manifest,
    );
    ...
    if (!runtimeProfile.mobile) sharpBand = new SharpBand(engine.scene, engine.renderer, chunkStore, manifest, miningController, loadPointIndexOnce);
    ...
    inventoryPanel = new InventoryPanel(app!, miningController.inventory, {
      ...
      pointsId: runtimeProfile.mobile ? null : resolvePointsId(datasetKey),
      metadataClient,
    ...
    if (metadataClient) {
      ...
      metadataFilters = new MetadataFilters(metadataClient, snapshot => {
        ...
        if (searchCompare && "setMetadataFilter" in searchCompare) searchCompare.setMetadataFilter(snapshot ? row => snapshot.matches(row) : null);
      });
    ...
    if (!runtimeProfile.mobile) void bootstrapMinimap(manifest, chunkStore, voxelProxy);
  } catch (error) {
...
engine.start((dt) => {
  ...
  if (holdTarget) {
    ...
    if (!stillHovering) {
      ...
      const durationSeconds = runtimeProfile.mobile ? 1.25 : EXTRACTION_CYCLE_MS / 1000;
      // On touch, do not collect an image the user hasn't been able to see yet.
      if (!touchPreview || holdElapsedSeconds < .2 || touchPreview.ready) holdElapsedSeconds += dt;
      if (touchPreview && holdElapsedSeconds >= .2 && miningController) {
        const mining = miningController;
        const state = mining.extractionState(holdTarget.chunkId, holdTarget.localVoxelId);
        const key = `${holdTarget.chunkId}:${holdTarget.localVoxelId}:${state?.cursor ?? 0}:${state?.extracted.size ?? 0}`;
        touchPreview.show(key, mining.viewCount(holdTarget.chunkId, holdTarget.localVoxelId) - mining.viewExtracted(holdTarget.chunkId, holdTarget.localVoxelId), async () => {
          const row = await mining.previewRowId(holdTarget.chunkId, holdTarget.localVoxelId);
          if (row === null) return null;
          const index = await loadPointIndexOnce(); await index.ensure?.(row); return resolveThumbUrl(index, row);
        });
      }
      ...
      if (holdElapsedSeconds >= durationSeconds) {
        ...
        if (cycle) {
          ...
          if (!runtimeProfile.mobile) launchExtractionFlight(cycle, hitPosition);
        }
        holdElapsedSeconds = 0;
        if (cycle?.complete) {
